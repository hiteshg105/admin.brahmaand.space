...
function EditContentWarZone() {
  ...
  const getSubcatdata = () => {
    axiosConfig
      .get(`/admin/list_by_subcategory_creator/${sub_category}`)
      .then((res) => {
  ...
  const editWar = async (e) => {
    ...
    if (
      ...
      if (data.data.status) {
        navigate.push("/app/brahmaand/content/creator/warzone/list");
  ...
  const getWar = async () => {
    const { id } = params;
    // console.log(id);
    const data = await axiosConfig.get(`/creator_warzone/single/${id}`);
          ...
          <Col sm="12">
            <div>
              <Breadcrumb listTag="div">
                ...
                <BreadcrumbItem
                  href="/app/brahmaand/content/creator/warzone/list"
                  tag="a"
                >
                  WarZone
                </BreadcrumbItem>
...
export default EditContentWarZone;
